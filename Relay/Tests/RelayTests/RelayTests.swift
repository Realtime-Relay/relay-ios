...
@Suite("Relay Tests")
final class RelayTests: @unchecked Sendable {
    ...
    private func connectWithRetry() async throws {
        ...
        throw lastError ?? NSError(domain: "RelayTests", code: -1, userInfo: [NSLocalizedDescriptionKey: "Failed to connect after \(retryCount) attempts"])
    }
    ...
    func testConnection() async throws {
        let result = try await relay.publish(subject: testSubject, message: "test")
    ...
    func testPublishSubscribe() async throws {
        ...
        // Publish a message
        _ = try await relay.publish(subject: testSubject, message: testMessage)
    ...
    func testJSONMessage() async throws {
        ...
        // Publish a message
        _ = try await relay.publish(subject: testSubject, object: testMessage)
    ...
    func testStream() async throws {
        ...
        for message in testMessages {
            _ = try await relay.publish(subject: testSubject, message: message)
        }
    ...
    private func withTimeout<T: Sendable>(seconds: UInt64, _ operation: @Sendable @escaping () async throws -> T) async throws -> T {
        try await withThrowingTaskGroup(of: T.self) { group in
            ...
            group.addTask {
                ...
                throw NSError(domain: "RelayTests", code: -1, userInfo: [NSLocalizedDescriptionKey: "Operation timed out after \(seconds) seconds"])
            }
